...
class Game extends Card {
  ...
  startGame() {
    ...
    _constants.readLine.question(`${betOptions.map((bet, index) => index + 1 + ". $" + bet)}\n[Press 1 to select $${betOptions[0]}, 2 to select $${betOptions[1]}, 3 to select $${betOptions[2]}]\n`, input => {
      switch (+input) {
        case 1:
...
function _generateBetOptions2() {
  if (_classPrivateMethodGet(this, _hasSufficientBalance, _hasSufficientBalance2).call(this)) {
    ...
    if (balance <= 500) {
      return _constants.chips.slice(0, 2);
    }
    let highestDenomination = _constants.chips[0];
    _constants.chips.forEach(chip => {
      if (balance > chip) {
        highestDenomination = chip;
      }
    });
    const chipIndex = _constants.chips.findIndex(chip => chip === highestDenomination);
    return _constants.chips.slice(chipIndex - 2, chipIndex + 1);
  }
}
...
function _canSplit2() {
  ...
  return this.firstServe && hasSufficientBalanceForSplit && _constants.commonCards.findIndex(card => card === this.playerCards[0][0]) >= 9 && _constants.commonCards.findIndex(card => card === this.playerCards[1][0]) >= 9 || this.firstServe && _constants.commonCards.findIndex(card => card === this.playerCards[0][0]) === _constants.commonCards.findIndex(card => card === this.playerCards[1][0]);
}
...
function _playSplit2() {
  ...
  this.firstSetSummationFromSplit = 0;
  const cardValue = _constants.commonCards.findIndex(card => card === firstSetCard[0]) + 1;
  if (cardValue === 1) {
  ...
  return function () {
    ...
    this.playerCardSummation = 0;
    const cardValue = _constants.commonCards.findIndex(card => card === secondSetCard[0]) + 1;
    if (cardValue === 1) {
...
function _playFaceDownCard2() {
  ...
  const cardValue = _constants.commonCards.findIndex(item => item === card) + 1;
  if (cardValue === 1) {
...
function _gameOver2() {
  _constants.readLine.question(`Play Again = 1, Exit = 2\n`, input => {
    switch (+input) {
      case 1:
